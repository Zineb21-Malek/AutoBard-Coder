...
quadrature or inversion of a mass matrix. We prove that the generating kernel
is a second-order mollifier, that the interpolatory reconstruction collocates
the unknown at the \dziti{} nodes, and that the semi-discrete Galerkin scheme
is unconditionally energy-stable. The corresponding explicit Euler scheme is
...
\item We distinguish clearly the $L^2$-orthogonal projection onto the
\dziti{} space from the interpolatory reconstruction at the roots, and we
correct the integral formulae that follow from that reconstruction.
\item We prove second-order consistency of the generating kernel as a
mollifier, interpolation at the \dziti{} nodes, energy stability of the
semi-discrete Galerkin scheme, and a CFL condition for the explicit Euler
method. In particular, the estimate previously claimed for a quasi-interpolant
built by evaluating the un-normalised generators at the mesh nodes is not
used: those generators do not form a partition of unity, and the corresponding
argument is invalid.
...
\label{lem:span}
Let $(\varphi_i)$ and $(\psi_i)$ be the families constructed in
Section~\ref{sec:1d}, and set $V_h:=\spanop\{\psi_i\}_{i=1}^{m+1}$. Then:
\begin{enumerate}
\item $\spanop\{\psi_i\}_{i=1}^{m+1}=\spanop\{\varphi_i\}_{i=1}^{m+1}$.
\item The two-term recurrence~\eqref{eq:GS} coincides with the full
Gram--Schmidt process. Consequently $(\psi_i)$ is orthonormal in $L^2(I)$.
\item For every $f\in L^2(I)$, the orthogonal projection
\begin{equation}
\label{eq:proj}
\Pi_h f:=\sum_{i=1}^{m+1}\langle f,\psi_i\rangle\,\psi_i
\end{equation}
is the unique best approximation of $f$ in $V_h$:
\begin{equation}
\label{eq:best}
\lVert f-\Pi_h f\rVert_{L^2(I)}
=\inf_{v\in V_h}\lVert f-v\rVert_{L^2(I)}.
\end{equation}
In particular, $\lVert f-\Pi_h f\rVert_{L^2(I)}\le\lVert f-I^\delta f\rVert_{L^2(I)}$
whenever $f\in C^0(I)$.
\end{enumerate}
\end{lemma}

\begin{proof}
(i) By~\eqref{eq:unfold}, each $\widetilde\psi_i$ (hence each $\psi_i$) lies
in $\spanop\{\varphi_1,\dots,\varphi_i\}$. Conversely, the inverse recurrence
\begin{equation}
\label{eq:inverse}
...
\quad i\ge 2,
\end{equation}
shows that each $\varphi_i$ lies in $\spanop\{\widetilde\psi_1,\dots,\widetilde\psi_i\}$.
The normalised family $(\psi_i)$ spans the same space because each $\psi_i$ is
a nonzero multiple of $\widetilde\psi_i$.

(ii) It is enough to show that $\widetilde\psi_i\perp\widetilde\psi_\ell$ for
all $\ell<i-1$; orthogonality to $\widetilde\psi_{i-1}$ is built into
$\lambda_{i-1}$. Proceed by induction. The claim is vacuous for $i=2$. Assume
it holds up to index $i-1$, so that
$(\widetilde\psi_\ell)_{\ell<i}$ is orthogonal and
$\widetilde\psi_\ell\in\spanop\{\varphi_1,\dots,\varphi_\ell\}$. For
$\ell\le i-2$ one has $\varphi_i\perp\varphi_j$ whenever $j\le\ell$, hence
$\varphi_i\perp\widetilde\psi_\ell$. Therefore
\begin{equation}
\langle\widetilde\psi_i,\widetilde\psi_\ell\rangle
=\langle\varphi_i,\widetilde\psi_\ell\rangle
+\lambda_{i-1}\langle\widetilde\psi_{i-1},\widetilde\psi_\ell\rangle=0.
\end{equation}
Normalisation gives orthonormality of $(\psi_i)$.

(iii) Let $v=\sum_i a_i\psi_i\in V_h$. Orthonormality yields
\begin{equation}
\lVert f-v\rVert_{L^2(I)}^2
=\lVert f\rVert_{L^2(I)}^2
-2\sum_{i=1}^{m+1}a_i\langle f,\psi_i\rangle
+\sum_{i=1}^{m+1}a_i^2.
\end{equation}
The quadratic is minimised if and only if $a_i=\langle f,\psi_i\rangle$, which
is~\eqref{eq:proj}. The comparison with $I^\delta f$ is the definition of the
orthogonal projection, since $I^\delta f\in V_h$.
\end{proof}

\begin{remark}
\label{rem:proj-vs-interp}
The map $f\mapsto I^\delta f$ from~\eqref{eq:interp} coincides with $\Pi_h$ on
$V_h$, but not on $C^0(I)$ in general. Using $c_i[f]=f(r_i)/\psi_i(r_i)$ as a
substitute for $\langle f,\psi_i\rangle$ is the collocation step of the
\dziti{} scheme. All consistency estimates below are stated for the operator
that is actually used.
\end{remark}

\subsection{Mollification error of the generating kernel}

The second-order accuracy advertised for the method is that of the mollifier
$\eta_h$, not that of the naive combination $\sum_i f(x_i)\varphi_i$. The
latter does not reproduce constants: the family $(\varphi_i)$ is \emph{not} a
partition of unity, since
$\sum_i\varphi_i(x)=(C/h)\sum_i\Phi((x-x_i)/h)$ is neither identically one
nor independent of $h$. A Taylor argument based on
$\sum_i\varphi_i\equiv 1$ is therefore not valid. The correct second-order
statement is the following.

\begin{theorem}[Second-order mollification]
\label{thm:mollifier}
Let $K(s):=C\Phi(s)$. Then $\int_{\mathbb{R}}K=1$, $K$ is even and compactly
supported in $(-1,1)$, and $\mu_2:=\int_{\mathbb{R}}s^2 K(s)\,\dd s\in(0,\infty)$.
If $f\in W^{2,\infty}(\mathbb{R})$, the convolution
$(\eta_h*f)(x)=\int_{\mathbb{R}}f(x-hs)K(s)\,\dd s$ satisfies
\begin{equation}
\label{eq:moll-err}
\lVert\eta_h*f-f\rVert_{L^\infty(\mathbb{R})}
\le\frac{\mu_2}{2}\,h^2\,\lVert f''\rVert_{L^\infty(\mathbb{R})}.
\end{equation}
The same bound holds on $I$ provided $f$ is extended to a function of class
$W^{2,\infty}$ in an $h$-neighbourhood of $I$.
\end{theorem}

\begin{proof}
Evenness of $K$ gives $\int s K(s)\,\dd s=0$. Taylor expansion with integral
remainder yields, for each $x$,
\begin{equation}
f(x-hs)-f(x)
=-hs\,f'(x)+\int_0^1(1-t)h^2 s^2 f''(x-ths)\,\dd t.
\end{equation}
Integrating against $K$ annihilates the first-order term, and
\begin{equation}
\bigl\lvert(\eta_h*f)(x)-f(x)\bigr\rvert
\le h^2\lVert f''\rVert_{L^\infty}
\int_{\mathbb{R}}\frac{s^2}{2}\,K(s)\,\dd s
=\frac{\mu_2}{2}h^2\lVert f''\rVert_{L^\infty}.
\end{equation}
\end{proof}

Thus every occurrence of $\eta_h$ as an approximate identity in the scheme
introduces an $\mathcal{O}(h^2)$ consistency error for $W^{2,\infty}$ data.
This is the precise sense in which the spatial approximation is second order.
A complete Jackson estimate for $\inf_{v\in V_h}\lVert f-v\rVert_{L^2(I)}$
would additionally require polynomial reproduction in $V_h$ (Strang--Fix
conditions). The space $V_h$ is spanned by $C^\infty$ bumps of width
$\mathcal{O}(h)$ and does not contain the constants, so we do not claim an
$h^2$ bound for $\lVert f-\Pi_h f\rVert_{L^2(I)}$ of finite-element type. In
the fully discrete scheme the unknown is represented by its values at the
nodes $(r_i)$, and the relevant spatial error is the consistency error of the
discrete Laplacian on that grid, which is $\mathcal{O}(h^2)$ on uniform
collocation meshes (Theorem~\ref{thm:scheme} below).
...
such that $u_h(0)=\Pi_h u_0$ (or $I^\delta u_0$) and
\begin{equation}
\label{eq:semidisc}
...
Combining Theorems~\ref{thm:mollifier} and~\ref{thm:scheme} with the
...
$V_h\subset L^2$. The roots of that basis define an interpolatory
reconstruction $I^\delta$ and a quadrature that avoid numerical integration
rules of Gaussian type. The $L^2$-orthogonal projection onto $V_h$ is the
best approximation in $V_h$, but it must not be confused with $I^\delta$, nor
with the combination $\sum_i f(x_i)\varphi_i$, which is not a partition-of-unity
quasi-interpolant.

The second-order character of the method is that of the generating mollifier
$\eta_h$ (Theorem~\ref{thm:mollifier}) and of the discrete Laplacian on the
collocation grid (Theorem~\ref{thm:scheme}). The semi-discrete Galerkin
scheme is unconditionally energy-stable; the fully discrete explicit Euler
scheme is stable under the three-dimensional CFL condition
$k\Delta t\le h_{\min}^2/(6c^2)$ and requires no mass-matrix inversion. The
same building blocks tensorise to the unit cube and extend to the unit ball
by spherical coordinates or by chord sweeping.

Several questions remain. A sharp Jackson estimate for
$\inf_{v\in V_h}\lVert f-v\rVert_{L^2}$ would require a modified generator
satisfying Strang--Fix conditions of order one; this lies outside the
original \dziti{} construction and is left to future work. The Neumann
closure on the ball, the treatment of variable or anisotropic conductivities,
and a fully discrete error analysis in $L^2(0,T;H^1(\Omega))$ are likewise
open. On the computational side, a comparison with PUFEM
\cite{Malek2019,Mohamed2013} at equal accuracy, including problems with steep
boundary layers, would quantify the cost reduction permitted by the explicit
\dziti{} update.
