...
\label{thm:semi}
Let $k>0$ be constant, $f\in L^2(0,T;L^2(\Omega))$ and
$g\in L^2(0,T;L^2(\partial\Omega))$. Let $u_h\in C^1([0,T];V_h(\Omega))$
solve~\eqref{eq:semidisc}. Then there exists a constant $C_T>0$, depending
only on $\Omega$, $k$ and $T$, such that for every $t\in[0,T]$,
\begin{equation}
\label{eq:energy}
\begin{aligned}
...
\end{aligned}
\end{equation}
In particular, the semi-discrete scheme is unconditionally stable in
$L^2(\Omega)$: the bound does not restrict $\Delta t$ or $h$.
\end{theorem}

\begin{proof}
Take $v_h=u_h(t)$ in~\eqref{eq:semidisc}. Since $a(u_h,u_h)=k\lVert\nabla u_h\rVert_{L^2(\Omega)}^2$,
\begin{equation}
\frac12\frac{\dd}{\dd t}\lVert u_h\rVert_{L^2(\Omega)}^2
+k\lVert\nabla u_h\rVert_{L^2(\Omega)}^2
=(f,u_h)+\langle g,u_h\rangle_{\partial\Omega}.
\end{equation}
The Cauchy--Schwarz inequality and the trace theorem yield
\begin{equation}
\begin{aligned}
...
\end{aligned}
\end{equation}
Young's inequality with parameters $1$ and $k/2$ gives
\begin{equation}
\begin{aligned}
...
\begin{theorem}[CFL stability of the explicit scheme]
\label{thm:scheme}
Let $L_h$ be the standard seven-point discrete Laplacian on a uniform mesh of
step $h$, with homogeneous Neumann (or periodic) closure. Then $L_h$ is
symmetric and negative semi-definite on $\ell^2(\mathcal{R}_h)$, and its
eigenvalues lie in $[-\Lambda_h,0]$ with $\Lambda_h=12/h^2$. Write
$\tau=k\Delta t$ and consider the homogeneous forward Euler scheme
\begin{equation}
\label{eq:euler-hom}
U^{n+1}=U^n+\tau L_h U^n=(I+\tau L_h)U^n.
\end{equation}
If
\begin{equation}
\label{eq:CFL}
...
\end{equation}
then $\lVert I+\tau L_h\rVert_{\ell^2\to\ell^2}\le 1$, and therefore
\begin{equation}
\label{eq:l2stab}
...
\end{equation}
On a quasi-uniform mesh with $c^{-1}h\le h_i\le c h$, the same argument holds
with~\eqref{eq:CFL} replaced by
$k\Delta t\le h_{\min}^2/(6c^2)$, where $h_{\min}:=\min_i h_i$.

If $f$ and $g$ are not zero, the same CFL condition yields
\begin{equation}
\label{eq:l2stab-inhom}
\lVert U^n\rVert_{\ell^2(\mathcal{R}_h)}
\le
\lVert U^0\rVert_{\ell^2(\mathcal{R}_h)}
+\Delta t\sum_{j=0}^{n-1}\lVert F^j\rVert_{\ell^2(\mathcal{R}_h)},
\end{equation}
where $F^j$ collects the source and boundary data at time $t_j$.
\end{theorem}

\begin{proof}
The operator $-L_h$ is symmetric positive semi-definite, so
$\lVert L_h V\rVert_{\ell^2}^2\le\Lambda_h\bigl(-(L_h V,V)_{\ell^2}\bigr)$
for every $V$, with $\Lambda_h=\lVert L_h\rVert_{\ell^2\to\ell^2}=12/h^2$
in three dimensions (the symbol is
$-(4/h^2)\sum_{\ell=1}^3\sin^2(\xi_\ell h/2)$). Expanding~\eqref{eq:euler-hom},
\begin{equation}
\begin{aligned}
\lVert U^{n+1}\rVert_{\ell^2}^2
&=\lVert U^n\rVert_{\ell^2}^2
+2\tau(L_h U^n,U^n)_{\ell^2}
+\tau^2\lVert L_h U^n\rVert_{\ell^2}^2 \\
&\le
\lVert U^n\rVert_{\ell^2}^2
+2\tau(L_h U^n,U^n)_{\ell^2}
+\tau^2\Lambda_h\bigl(-(L_h U^n,U^n)_{\ell^2}\bigr) \\
&=\lVert U^n\rVert_{\ell^2}^2
+\bigl(2\tau-\tau^2\Lambda_h\bigr)(L_h U^n,U^n)_{\ell^2}.
\end{aligned}
\end{equation}
The inner product $(L_h U^n,U^n)_{\ell^2}$ is nonpositive. Hence
$\lVert U^{n+1}\rVert_{\ell^2}\le\lVert U^n\rVert_{\ell^2}$ as soon as
$2\tau-\tau^2\Lambda_h\ge 0$, that is $\tau\le 2/\Lambda_h=h^2/6$, which is
\eqref{eq:CFL}. On a quasi-uniform mesh one has
$\Lambda_h\le 12c^2/h_{\min}^2$, and the same computation gives
$k\Delta t\le h_{\min}^2/(6c^2)$.

In the inhomogeneous case, $U^{n+1}=(I+\tau L_h)U^n+\Delta t F^n$. Under the
CFL condition, $\lVert I+\tau L_h\rVert\le 1$, therefore
$\lVert U^{n+1}\rVert\le\lVert U^n\rVert+\Delta t\lVert F^n\rVert$. Summing
the resulting telescoping inequality yields~\eqref{eq:l2stab-inhom}.
\end{proof}
...
\(2d+1\) points. On the ball, \(h_{\min}\) is the smallest spacing among the
radial and angular collocation nodes, multiplied by the appropriate metric
factors near the origin. The condition is independent of the orthonormal
\dziti{} basis: it is a restriction on the explicit Euler discretisation of
