...
interpolant $I_h f=\sum_i f(x_i)\varphi_i(x)$. A second-order $L^2$ bound
on that interpolant holds if the generators reproduce constants and
linears; those identities are not implied by the mollifier family used here. The semi-discrete Galerkin scheme is unconditionally
...
$I_h f=\sum_{i}f(x_i)\varphi_i(x)$. The Taylor expansion of $I_h f$ yields
an $O(h^2)$ bound if the generators satisfy the first-order Strang--Fix
identities; these identities fail for the family~\eqref{eq:phi}.
...
interpolant~\eqref{interp}. The next result records the exact Taylor
remainder of $I_h f$ and isolates the identities that would turn the
comparison into an $O(h^2)$ bound.

\begin{theorem}[Comparison and conditional error estimate]
\label{thm:error}
...
\lVert f-I_h f\rVert_{L^2(I)}.
\end{equation}
Moreover, for every $x\in I$ there exist intermediate points $\xi_i(x)$ such
that
\begin{equation}
\label{eq:taylor}
\begin{aligned}
f(x)-I_h f(x)
&=
f(x)\Biggl(1-\sum_{i=1}^{m+1}\varphi_i(x)\Biggr)
-f'(x)\sum_{i=1}^{m+1}(x_i-x)\varphi_i(x)\\
&\qquad
-\frac12\sum_{i=1}^{m+1}f''\bigl(\xi_i(x)\bigr)(x_i-x)^2\varphi_i(x).
\end{aligned}
\end{equation}
If, in addition, the generators satisfy the first-order Strang--Fix
identities
\begin{equation}
\label{eq:SF}
\sum_{i=1}^{m+1}\varphi_i(x)=1,
\qquad
\sum_{i=1}^{m+1}(x_i-x)\varphi_i(x)=0
...
\begin{proof}
The comparison~\eqref{eq:err-comp} is Lemma~\ref{lem:orthonormal}(ii), since
$I_h f\in V_h$ by~\eqref{eq:spans}.

For~\eqref{eq:taylor}, Taylor expansion with remainder at each node gives
\begin{equation}
f(x_i)=f(x)+f'(x)(x_i-x)+\tfrac12 f''\bigl(\xi_i(x)\bigr)(x_i-x)^2,
\end{equation}
with $\xi_i(x)$ between $x$ and $x_i$. Multiply by $\varphi_i(x)$ and sum
over $i$:
\begin{equation}
\begin{aligned}
I_h f(x)
&=
f(x)\sum_i\varphi_i(x)
+f'(x)\sum_i(x_i-x)\varphi_i(x)\\
&\qquad
+\frac12\sum_i f''\bigl(\xi_i(x)\bigr)(x_i-x)^2\varphi_i(x).
\end{aligned}
\end{equation}
Rearrangement is~\eqref{eq:taylor}.

Now assume~\eqref{eq:SF}. The first two sums on the right-hand side of
\eqref{eq:taylor} vanish, and $\lvert x_i-x\rvert\le h$ on
$\supp(\varphi_i)$. The generators~\eqref{eq:phi} are nonnegative, so
\eqref{eq:SF}$_1$ implies $\sum_i\lvert\varphi_i(x)\rvert=1$. Hence
\begin{equation}
\lvert f(x)-I_h f(x)\rvert
\le
\tfrac12 h^2\lVert f''\rVert_{L^\infty(I)}.
\end{equation}
Squaring, integrating over $I$, and taking the square root yields
\begin{equation}
\lVert f-I_h f\rVert_{L^2(I)}
\le
\tfrac12\lvert I\rvert^{1/2}h^2\lVert f''\rVert_{L^\infty(I)}.
\end{equation}
Combined with~\eqref{eq:err-comp} this is~\eqref{eq:err}.
\end{proof}

\begin{remark}
\label{rem:SF-false}
The identities~\eqref{eq:SF} are hypotheses, not properties of the family
\eqref{eq:phi}. At a mesh node $x_j$ one has $\varphi_i(x_j)=0$ for
$i\neq j$, hence
\begin{equation}
\sum_{i=1}^{m+1}\varphi_i(x_j)
=\varphi_j(x_j)
=\frac{C}{he}\neq 1.
\end{equation}
(The value $\varphi_j(x_j)=C/(he)$ is the nodal Kronecker identity for these
generators.) Thus~\eqref{eq:SF}$_1$ fails, and the $O(h^2)$ conclusion
\eqref{eq:err} is not available for this interpolant. The comparison
\eqref{eq:err-comp} remains valid independently of~\eqref{eq:SF}.

The continuous identities for the kernel do not repair this. From
\eqref{eq:mollifier} one has $\int_{\mathbb{R}}\eta_h=1$ and, by evenness,
$\int_{\mathbb{R}}s\,\eta_h(s)\,\dd s=0$, or equivalently
\begin{equation}
\label{eq:cont-SF}
\int_{\mathbb{R}}\eta_h(x-y)\,\dd y=1,
\qquad
\int_{\mathbb{R}}(y-x)\eta_h(x-y)\,\dd y=0.
\end{equation}
These are integral identities for the mollifier, not summation identities for
the nodal translates. A Riemann-sum analogue would replace $\mathrm{d}y$ by
the mesh weight $h$, and would read
$\sum_i\varphi_i(x)\,h\approx 1$, that is
$\sum_i\varphi_i(x)\approx 1/h$, which is not~\eqref{eq:SF}$_1$. Moreover the
support of $\eta_h(x-\,\cdot\,)$ has width $2h$, so only two nodes
$x_k,x_{k+1}$ contribute. For $x=x_k+\theta h$ with $\theta\in(0,1)$,
\begin{equation}
\label{eq:discrete-mass}
\sum_{i}\varphi_i(x)
=\frac{C}{h}\bigl(\Phi(\theta)+\Phi(1-\theta)\bigr),
\qquad
\sum_{i}(x_i-x)\varphi_i(x)
=C\bigl(-\theta\Phi(\theta)+(1-\theta)\Phi(1-\theta)\bigr).
\end{equation}
Neither right-hand side is independent of $\theta$: the first is of order
$h^{-1}$, and the second vanishes at midpoints $\theta=1/2$ but not in
general. Thus~\eqref{eq:cont-SF} cannot be converted into~\eqref{eq:SF} by
replacing integrals with sums.
\end{remark}
...
$I_h f=\sum_{i=1}^{m+1}f(x_i)\,\varphi_i(x)$. Theorem~\ref{thm:error} records
the comparison $\lVert f-f_h\rVert_{L^2}\le\lVert f-I_h f\rVert_{L^2}$ and
shows that an $O(h^2)$ bound on the right-hand side requires the first-order
Strang--Fix identities, which fail for the generators~\eqref{eq:phi}.
