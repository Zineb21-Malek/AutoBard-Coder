...
\label{eq:best}
\lVert f-f_h\rVert
=\inf_{v\in V_h}\lVert f-v\rVert
\le
\lVert f-I_h f\rVert,
\end{equation}
where $I_h f$ is the combination of the generators associated with the mesh
nodes,
\begin{equation}
\label{interp}
I_h f=\sum_{i=1}^{m+1}f(x_i)\,\varphi_i.
\end{equation}
\end{enumerate}
...
\end{equation}
From (i), the combination $I_h f$ defined by~\eqref{interp} belongs to $V_h$.
Taking $v=I_h f$ in the infimum gives the comparison
$\lVert f-f_h\rVert_{L^2(I)}\le\lVert f-I_h f\rVert_{L^2(I)}$.
...
$h^2$ bound for $\lVert f-f_h\rVert_{L^2(I)}$ of finite-element type. In
...
least-squares approximation in $V_h$, and in particular it improves upon the
generator combination $I_h f=\sum_i f(x_i)\varphi_i$.
